...
import com.app.imagerandom.domain.usecase.people.GetPersonUseCase
            ...
            try {
                getPersonUseCase.getPerson(language, currentPage).collect { result ->
                    when (result) {
                        ...
                        is Response.Success -> {
                            val incoming = result.data ?: PersonResponse(page = currentPage, results = emptyList(), totalPage = 10)
